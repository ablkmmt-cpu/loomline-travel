(function () {
  ...
  var cssHref = script ? new URL("whatsapp-float.css?v=8", script.src).href : "whatsapp-float.css?v=8";
  ...
  function getPageMessage() {
    ...
    if (match && destinationNames[match[1]]) {
      return "Hi TripToChina, I'm interested in adding " + destinationNames[match[1]] + " to my China trip. Could you help me with an itinerary?";
    }

    match = path.match(/\/experiences\/([^/]+)\//);
    if (match && experienceNames[match[1]]) {
      return "Hi TripToChina, I'd like to add the " + experienceNames[match[1]] + " to my China trip. Could you tell me more about availability?";
    }

    if (path.indexOf("/services/self-guided/") !== -1) {
      return "Hi TripToChina, I'm interested in Self-Guided Trip Support. Could you tell me how it would work for my trip?";
    }

    if (path.indexOf("/services/custom-tour/") !== -1) {
      return "Hi TripToChina, I'm interested in a Custom Tour Package. Could you help me plan a private trip and prepare a quote?";
    }

    if (path.indexOf("/field-guide-taste-test/") !== -1) {
      return "Hi TripToChina, I'm exploring your China trip planning approach and have a question.";
    }

    if (path.indexOf("/field-guide/") !== -1) {
      return "Hi TripToChina, I'm reading your China travel field guide and would like some local advice.";
    }

    return "Hi TripToChina, I'm planning my first trip to China and would like some local help.";
  }
  ...
  function buildGlobalHeader() {
    ...
    header.innerHTML =
      '<a class="ttc-global-brand" href="' + homeUrl + '#top" aria-label="TripToChina home">' +
        '<img src="' + new URL("assets/logo3-emblem-gold.png", siteRoot).href + '" alt="">' +
        '<img src="' + new URL("assets/logo3-wordmark-ivory.png", siteRoot).href + '" alt="TripToChina">' +
      '</a>' +
      ...
      '<div class="ttc-nav-actions">' +
        '<a class="ttc-nav-whatsapp" href="' + whatsappUrl + '" target="_blank" rel="noopener noreferrer" aria-label="Chat with TripToChina on WhatsApp">' + whatsappIcon + '</a>' +
  ...
  function enhanceNavigation() {
    ...
    actions.appendChild(makeWhatsappLink("ttc-nav-whatsapp", "Chat with TripToChina on WhatsApp"));
  ...
  function initializeWhatsappFloat() {
    ...
    var hasMobileCta = document.querySelector(".tea-mobile-cta, .chengdu-mobile-cta");
